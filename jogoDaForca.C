...
void adicionarPalavra(){
    ...
    temp=fecharArq(temp);
    if(erro==true) {
        printf("znNAO FOI POSSIVEL ENCONTRAR DICA!");
        printf("znAPERTE UMA TECLA PARA VOLTAR AO MENU...");
        fflush(stdin);
        resp=getchar();
        return;
    }
    ...
    if(fwrite(&guarda,sizeof(guarda),1,temp)!=1){
        printf("\nNAO FOI POSSSIVEL ADICIONAR A PALAVRA!");
        printf("\nAPERTE UAM TECLA PARA VOLTAR AO MENU...!");
        fflush(stdin);
        resp=getchar();
        return;
    }
...
FILE *fecharArq(FILE *arq){
    if(arq==NULL){
        printf("\bErro ao fechar o arquivo!\n");
    }
    fclose(arq);
    return arq;
}
